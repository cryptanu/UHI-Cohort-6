...
contract PointsHook is BaseHook, ERC1155 {
    ...
    /// @notice Allows a user to redeem (burn) their points for a given pool
    function redeemPoints(uint256 poolId, uint256 amount) external {
        require(this.balanceOf(msg.sender, poolId) >= amount, "Not enough points");
        _burn(msg.sender, poolId, amount);

        // Conversion rate: 1 point = 1 wei (adjust as needed)
